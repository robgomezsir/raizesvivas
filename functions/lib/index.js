...
export const beforeUserCreated = beforeUserCreatedFn(async (event) => {
    ...
    if (!email) {
        throw new HttpsError("failed-precondition", "E-mail é obrigatório para cadastro.");
    }
    ...
    // (chegamos aqui se não encontrou convite OU se encontrou mas expirou)
    {
        ...
        if (!usersSnap.empty) {
            ...
            logger.info(`Permitindo recriação de conta para usuário existente: ${email} ` +
                `(Admin: ${isAdmin})`);
            return;
        }
        // Caso contrário, bloquear (apenas novos usuários precisam de convite)
        throw new HttpsError("permission-denied", "Cadastro permitido apenas por convite ou aprovação do administrador.");
    }
});
/**
